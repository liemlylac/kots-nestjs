...
describe('class HashService', () => {
  ...
  beforeAll(async () => {
    ...
    obj.hashPassword = await bcrypt.hash(obj.password + obj.pepper, obj.saltRounds);
  });
  ...
  describe('hashPassword()', () => {
    ...
    it('should return false if input wrong password', async () => {
      expect(await hashService.compareHash('someWrongPassword', obj.hashPassword)).toBe(false);
    });
  });
});
